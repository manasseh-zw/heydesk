...
type AppleDownloadButtonProps = Omit<React.ComponentProps<"a">, "children">;

export function AppleDownloadButton({
  className,
  ...props
}: AppleDownloadButtonProps) {
  return (
    <a
      className={cn(
        "inline-flex h-12 items-center gap-3 rounded-xl bg-black px-5 text-white shadow-lg shadow-slate-300/40 transition hover:-translate-y-0.5 hover:bg-slate-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black",
        className,
      )}
      {...props}
    >
      <AppleIcon className="size-5 shrink-0" aria-hidden="true" />
      <span className="text-sm font-medium">Download For Mac</span>
    </a>
  );
}
